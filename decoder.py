...
def decode_airborne_velocity(hex_msg: str) -> Dict[str, Any]:
    ...
    if df not in (17, 18):
        raise ValueError("Message must be Downlink Format 17 or 18 (Extended Squitter).")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"  - Even Frame Position: ({pos_res['latitude_even']:.6f}, {pos_res['longitude_even']:.6f})")
        print(f"  - Odd Frame Position:  ({pos_res['latitude_odd']:.6f}, {pos_res['longitude_odd']:.6f})")
        print(f"  - Even Frame Altitude: {pos_res['altitude_even']} ft")
        print(f"  - Odd Frame Altitude:  {pos_res['altitude_odd']} ft")
        print(f"  - Distance Moved:      Approx. {abs(pos_res['latitude_even'] - pos_res['latitude_odd'])*111.1:.3f} km north-south")
    ...
    for label, msg in [("Subtype 1 (Ground Speed)", sub1_test), ("Subtype 3 (Airspeed)", sub3_test)]:
        print(f"\nDecoding {label}: {msg}")
        try:
            ...
            if vel_res['east_west_velocity'] is not None:
                print(f"  - E-W component:      {vel_res['east_west_velocity']:.2f} knots")
            if vel_res['north_south_velocity'] is not None:
                print(f"  - N-S component:      {vel_res['north_south_velocity']:.2f} knots")
            if vel_res['vertical_rate'] is not None:
                print(f"  - Vertical Rate:      {vel_res['vertical_rate']} ft/min ({vel_res['vertical_rate_source']})")
